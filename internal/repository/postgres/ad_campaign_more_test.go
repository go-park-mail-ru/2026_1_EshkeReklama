...
func TestAdCampaignRepository_CreateAndGetByID(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(insertAdCampaign)).
		WithArgs(c.AdvertiserID, c.Status, c.Name, c.DailyBudget).
	...
	mock.ExpectQuery(regexp.QuoteMeta(selectAdCampaignByID)).
		WithArgs(3).
		WillReturnRows(sqlmock.NewRows([]string{"id", "advertiser_id", "status", "name", "daily_budget", "created_at", "updated_at"}).
