...
func TestAdvertiserRepository_GetByID_Update(t *testing.T) {
	...
	now := time.Now()
	cols := []string{"id", "name", "surname", "avatar_url", "balance", "company", "city", "tariff", "role", "created_at", "updated_at"}

	row := sqlmock.NewRows(cols).
		AddRow(1, "n", "", sql.NullString{}, int64(10), "", "", "noob", "user", now, sql.NullTime{})
	...
	mock.ExpectExec(regexp.QuoteMeta(updateAdvertiser)).
		WithArgs(adv.Name, adv.Surname, adv.AvatarURL, adv.Balance, adv.Company, adv.City, adv.Tariff, adv.Role, adv.ID).
